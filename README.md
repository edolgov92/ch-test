...
#### Logs with Elastic Cloud

With Elastic Cloud, you can seamlessly integrate `Kubernetes` logging by choosing the `"Collect Kubernetes container logs"` option. This will necessitate the installation of agents onto your AWS EKS cluster. To proceed with the setup, simply follow the on-screen instructions when adding `Kubernetes` integration.
...
- **Default Filter**: Use `event.dataset: kubernetes.container_logs` to focus on container logs generated by Kubernetes.
